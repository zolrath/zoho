...
CRM.prototype.searchRecords = function (module, params, callback) {
  ...
  if (typeof params !== 'object' || !params.criteria) {
    return callback({ message: 'Error: Criteria required parameter missing to search records' }, null);
  }
...
CRM.prototype.createRecord = function (module, params, callback) {
  ...
  this._request('POST', module + '/insertRecords', { xmlData: xml }, callback);
};
...
CRM.prototype.createNote = function (id, title, content, callback){
  ...
  this._request('POST', mod + '/insertRecords', { xmlData: xml }, callback);
};

/**
* Get a specific Note via noteid
* Builds xml and sends getSearchRecordsByPDC request for 'Notes'
* @param {String} id - Note identificator
* @param {Function} callback -
*/
CRM.prototype.getNote = function (id, callback){
  var mod = 'Notes';
  var params =  {
 ...
 * Builds xml and sends getRelatedRecords request for 'Notes'
 * @param {String} parentModule - Parent Zoho module
 * @param {String} id - Object identificator
 * @param {Function} callback -
 */
CRM.prototype.getNotes = function (parentModule, id, callback){
    var mod = 'Notes';
    var params =  {
        parentModule: parentModule,
        id: id
    };
    this._request('GET', mod + '/getRelatedRecords', params, callback);
};

/**
 * Get all attachments for a specific object
 * Builds xml and sends getRelatedRecords request for 'Attachments'
 * @param {String} parentModule - Parent Zoho module
 * @param {String} id - Object identificator
 * @param {Function} callback -
 */
CRM.prototype.getAttachments = function (parentModule, id, callback){
    var mod = 'Attachments';
    var params =  {
        parentModule: parentModule,
        id: id
...
CRM.prototype._request = function (method, endpoint, params, callback) {
  ...
  if (method == 'POST') {
    ...
    urlSuffix = '?' + qs.stringify(params);
  }

  var options = {
    host: this.host,
    port: this.port,
    path: '/crm/private/json/' + endpoint + urlSuffix,
    method: method,
    headers: headers
  };
  ...
  var req = protocol.request(options, function (res) {
    ...
    res.on('end', function () {
      if (data) {
        //todo check http code before trying to parse
        try {
